...
function verifiedSignature(rawBody, receivedSignature) {
...
export default async (request) => {
  ...
  if (!verifiedSignature(rawBody, request.headers.get('x-signature'))) return plain(401, 'Invalid webhook signature.');
  try {
    ...
    if (error) {
      if (/compatible key|not configured/i.test(error.message)) return plain(400, 'We are currently out of stock. Please wait for restock.');
      throw error;
    }

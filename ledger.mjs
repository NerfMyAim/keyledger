...
async function adminClient(request) {
  ...
  return {
    server: client,
    actor: createClient(url, serviceRoleKey, { global: { headers: { Authorization: authorization } } })
  };
}

export default async (request) => {
  try {
    const clients = await adminClient(request);
    const server = clients.server;
    const actor = clients.actor;
    if (request.method === 'GET') {
      const [inventory, assignments] = await Promise.all([
        server.from('inventory_keys').select('*').order('created_at'),
        server.from('assignments').select('*, inventory_keys(key_value, license_type)').order('assigned_at', { ascending: false })
      ]);
    ...
    if (request.method === 'POST') {
      ...
      const { data, error } = await actor.rpc('assign_inventory_key', {
        p_key_id: body.keyId,
    ...
    if (request.method === 'PATCH') {
      ...
      if (body.action === 'finish') {
        const { error } = await actor.rpc('finish_assignment', { p_assignment_id: body.assignmentId });
        if (error) return response(409, { error: error.message });
        return response(200, { ok: true });
      }
      if (body.action === 'note') {
        const { error } = await actor.rpc('update_key_note', { p_key_id: body.keyId, p_note: body.note || '' });
        if (error) return response(409, { error: error.message });
        return response(200, { ok: true });
      }
      if (body.action === 'release-key') {
        const { error } = await actor.rpc('release_inventory_key', { p_key_id: body.keyId, p_note: body.note || '' });
        if (error) return response(409, { error: error.message });
        return response(200, { ok: true });
      }
      if (body.action === 'add-key') {
        ...
        const { data: key, error } = await server.from('inventory_keys').insert({
        ...
        await server.from('inventory_events').insert({ key_id: key.id, event_type: 'manual_add', note: body.note?.trim() || '' });
        return response(201, { ok: true });
      }
      if (body.action === 'update-assignment') {
        const { error } = await server.from('assignments').update({
